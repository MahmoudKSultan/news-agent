...
export async function fetchRssFeed(source: Source): Promise<ParsedArticle[]> {
  try {
    ...
    return feed.items.map((item) => ({
      ...
      content: truncate(item.contentSnippet?.trim() || item.content?.trim() || "", 50000),
...
export async function fetchAllSources(sessionId: string): Promise<{ source: string; count: number }[]> {
